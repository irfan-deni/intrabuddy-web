...
function StudentFormModal({
  ...
  loading: boolean;
  onCancel: () => void;
  onSubmit: (values: StudentFormInput) => void;
}) {
  const [form, setForm] = useState<StudentFormInput>(defaultValues);

  const updateField = (field: keyof StudentFormInput, value: string) => {
    ...
    <div className="fixed inset-0 z-50 grid place-items-center bg-black/40 p-4">
      <div className="w-full max-w-lg rounded bg-white p-6 shadow-xl">
        ...
        <div className="mt-4 grid gap-3">
          <input className="rounded border px-3 py-2" placeholder="Student ID" value={form.student_id} onChange={(e) => updateField("student_id", e.target.value)} />
          <input className="rounded border px-3 py-2" placeholder="Name" value={form.name} onChange={(e) => updateField("name", e.target.value)} />
          <input className="rounded border px-3 py-2" placeholder="Email" value={form.email} onChange={(e) => updateField("email", e.target.value)} />
          <input className="rounded border px-3 py-2" placeholder="Cohort" value={form.cohort} onChange={(e) => updateField("cohort", e.target.value)} />
        </div>
        <div className="mt-6 flex justify-end gap-2">
          <button className="rounded border px-4 py-2" onClick={onCancel}>Cancel</button>
          <button
  ...
  return (
    <div className="fixed inset-0 z-50 grid place-items-center bg-black/40 p-4">
      <div className="w-full max-w-md rounded bg-white p-6 shadow-xl">
        ...
        <p className="mt-2 text-sm text-slate-700">Are you sure you want to delete {name}?</p>
        <div className="mt-6 flex justify-end gap-2">
          <button className="rounded border px-4 py-2" onClick={onCancel}>Cancel</button>
          <button
